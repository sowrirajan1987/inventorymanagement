...
import { Search, Download, Package, Loader2, Check, ChevronsUpDown, User } from "lucide-react";
...
function MobileDashboardCard({ item }: { item: DashboardItem }) {
  return (
    <div className="flex gap-3 p-4 bg-card border border-border rounded-lg">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        {item.project && item.project !== "Not Applicable" && (
        ...
        <div className="text-sm text-muted-foreground">
          ...
          <p>Inward: {item.inventory || 0} | Outward: {item.itemissued || 0} | Balance: {item.balance || 0}</p>
        </div>
      </div>
    </div>
  );
...
export function DashboardTable() {
  ...
  const [rowData, setRowData] = useState<DashboardItem[]>([]);
  ...
  const [displayedData, setDisplayedData] = useState<DashboardItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        formData.append("tabletype", "inventory");
        formData.append("filtereddepartment", departmentFilter);
        formData.append("project", projectFilter);
        formData.append("search", searchQuery);

        const res = await fetchApi(`${API_BASE_URL}/api/dashboardtable`, {
          method: "POST",
          body: formData,
        });
  ...
  const columnDefs: ColDef<DashboardItem>[] = [
    ...
    { field: "inventory", headerName: "Inward", width: 100, cellClass: "text-center" },
    { field: "itemissued", headerName: "Outward", width: 100, cellClass: "text-center" },
    { field: "balance", headerName: "Closing", width: 100, cellClass: "text-center" },
  ];

  const exportToExcel = () => {
    const headers = ["Product", "Department", "Project", "Inward", "Outward", "Closing"];
      ...
      <div className="md:hidden space-y-3">
        {loading ? (
          ...
          displayedData.map((item, idx) => (
            <MobileDashboardCard key={`dashboard-${item.id}-${idx}`} item={item} />
          ))
        )}
      </div>
